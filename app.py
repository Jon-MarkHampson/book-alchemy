...
def home():
    ...
    for book in books:
        ...
        if not cover_url or not synopsis:
            ...
            if book.isbn and (not cover_url or not synopsis):
                google_books_api_url_isbn = f"https://www.googleapis.com/books/v1/volumes?q=isbn:{book.isbn}"
                try:
                    response = requests.get(
                        google_books_api_url_isbn, timeout=5)
                    ...
                    if data.get('totalItems', 0) > 0 and 'items' in data:
                        ...
                        if not cover_url:
                            ...
                            new_cover_url = image_links.get(
                                'thumbnail') or image_links.get('smallThumbnail')
                            if new_cover_url:
            ...
            if not cover_url and book.isbn:
                ...
                print(
                    f"Google Books failed for '{book.title}', trying Open Library...")
                # Add default=false
                ...
                try:
                    ...
                    if head_response.status_code == 200 and 'image' in head_response.headers.get('Content-Type', '').lower():
                        ...
                        if redirect_url and 'notfound' not in redirect_url.lower() and 'placeholder' not in redirect_url.lower():
                            get_response = requests.get(
                                open_library_cover_url, timeout=3, allow_redirects=True)
                            if get_response.status_code == 200 and 'image' in get_response.headers.get('Content-Type', '').lower():
            ...
            if not cover_url or not synopsis:
                print(
                    f"Trying Google Books by Title+Author for '{book.title}'...")
                ...
                try:
                    response = requests.get(
                        google_books_api_url_title, timeout=5)
                    response.raise_for_status()
                    data = response.json()
                    if data.get('totalItems', 0) > 0 and 'items' in data:
                        volume_info = data['items'][0].get('volumeInfo', {})
    ...
    if invalid_isbns:
        isbn_links = [
            f'<a href="{url_for('book_detail', book_id=item['id'])}">{item['title']} ({item['isbn']})</a>' for item in invalid_isbns]
        flash(
            f"Warning: Found {len(invalid_isbns)} books with potentially invalid ISBN formats: {', '.join(isbn_links)}. Check console logs.", 'warning')
...
def author_detail(author_id):
    ...
    author = Author.query.options(
        db.joinedload(Author.books)).get_or_404(author_id)
...
def remove_cover(book_id):
    ...
    if book.cover_url:
        ...
        db.session.commit()
        flash(
            f'Cover removed for "{book.title}". It will be re-fetched on next load.', 'success')
    else:
...
def delete_author(author_id):
    ...
    try:
        ...
        db.session.commit()
        flash(
            f'Author "{author_name}" and all associated books deleted successfully.', 'success')
        return redirect(url_for('home'))
    except Exception as e:
        db.session.rollback()
...
def add_author():
    if request.method == 'POST':
        ...
        # Get bio from form
        bio = request.form.get('bio')  # Use .get to handle if it's missing
        ...
        # Include bio when creating new author
        new_author = Author(name=name, birth_date=birthdate_obj,
                            death_date=date_of_death_obj, bio=bio)
        db.session.add(new_author)
        db.session.commit()
        flash('Author added successfully!', 'success')
        # Redirect to the new author's detail page might be nice, but redirecting back to add form is simpler
...
def add_book():
    ...
    if request.method == 'POST':
        ...
        try:
            if publication_date_str:
                publication_date_obj = datetime.strptime(
                    publication_date_str, '%Y-%m-%d').date()
        except ValueError:
        ...
        new_book = Book(isbn=isbn, title=title, publication_date=publication_date_obj,
